...
public abstract class OGSugarCaneBlockBase extends Block
{
    ...
    public OGSugarCaneBlockBase(Settings settings) {
        super(settings);
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random)
    {
        if (!world.getDimensionEntry().matchesId(DimensionTypes.THE_END_ID) && this.canPlaceAt(state, world, pos))
        {
            if (world.isAir(pos.up()))
            {
                ...
                while (reedHeight < 3 && world.getBlockState(posAtBase).getBlock() instanceof OGSugarCaneBlockBase)
                {
                    reedHeight++;
